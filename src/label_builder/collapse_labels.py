...
class CollapseLabelBuilder:
    ...
    def build_labels_for_outing(self, outing_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        if pa_df.empty:
            ...
            df["window_blended_xwoba"] = np.nan
            df["collapse_reason"] = pd.NA
        ...
        def describe_reason(row):
            reasons = []
            if row["rolling_xwoba"] >= self.blended_xwoba_threshold:
                reasons.append("xwOBA")
            if row["rolling_barrels"] >= self.min_barrels_in_window:
                reasons.append("barrels")
            if row["rolling_bb_hbp"] >= self.min_bb_hbp_in_window:
                reasons.append("BB/HBP")
            return "+".join(reasons) if reasons else pd.NA

        pa_df["collapse_reason"] = pa_df.apply(describe_reason, axis=1)
            ...
            if row["is_bad_window"]:
                if not in_episode:
                    ...
                    curr_ep = {
                        ...
                        "window_blended_xwoba": row["rolling_xwoba"],
                    }
                    episode_counter += 1
                else:
        ...
        if not episodes_df.empty:
            ...
            episodes_df["game_date"] = df["game_date"].iloc[0]
            if "dataset_split" in df:
                episodes_df["dataset_split"] = df["dataset_split"].iloc[0]
            if "actual_data_source" in df:
                episodes_df["actual_data_source"] = df["actual_data_source"].iloc[0]
        ...
        xwoba_by_pa = pa_df.set_index("pa_number_in_outing")["rolling_xwoba"]
        reason_by_pa = pa_df.set_index("pa_number_in_outing")["collapse_reason"]
        df["window_blended_xwoba"] = df["pa_number_in_outing"].map(xwoba_by_pa)
        df["collapse_reason"] = df["pa_number_in_outing"].map(reason_by_pa)
