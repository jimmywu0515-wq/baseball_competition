...
class CaseStudyVisualizer:
    ...
    def plot_case_study(self, 
        ...
        alert_pitches = df[df.get("is_cusum_alert", False)]["pitch_number_in_outing"].values
        ...
        ax1.set_title(f"[{case_type}] {pitcher_name} | {game_date} (Game PK: {game_pk})\n{title_note}", fontsize=13, fontweight="bold", pad=12)

        if first_alert:
            ax1.axvline(first_alert, color="#d81b60", linestyle="-", linewidth=2, label=f"CUSUM Alert (Pitch #{first_alert})", zorder=4)
            ax1.scatter([first_alert], [msi[first_alert-1]], color="#d81b60", s=100, zorder=5)
        ...
        if not fb_df.empty:
            ax3.scatter(fb_df["pitch_number_in_outing"], fb_df["release_speed"], color="#3949ab", alpha=0.4, s=30, label="Fastball Speed")
            ax3.plot(fb_df["pitch_number_in_outing"], fb_df["release_speed"].rolling(4, min_periods=1).mean(), color="#3949ab", linewidth=2.5, label="Rolling Fastball Velocity (mph)")
        else:
        ...
        # -------------------------------------------------------------
        ax4 = axes[3]
        xwoba_series = df.get("window_blended_xwoba", pd.Series(np.zeros(len(df))))
        ax4.plot(pitch_nums, xwoba_series, color="#c2185b", linewidth=2.5, label="Rolling 3-PA Blended xwOBA")
